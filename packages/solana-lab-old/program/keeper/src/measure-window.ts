 ...
 * P&L, or a win from the skim. A real settle is {Ed25519, nuvem, System}; a
 * trade bundled with a nuvem instruction still carries Jupiter/Raydium/Token in
 * its program set, so exclusivity keeps it trading. Asserted at image-build
 * time (supervisor --preflight), so a regression fails the Docker build.
 */
...
export async function measureSince(
   ...
   * link was created — in that case; see settle-tick.
   */
  frontierSlot: bigint,
  /**
   * The nuvem-vault program id. A transaction that invokes it from this wallet
