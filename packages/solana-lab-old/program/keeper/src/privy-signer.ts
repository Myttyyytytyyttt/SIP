 ...
 * (nothing vs. re-running the onboarding registration). The EVM keeper's
 * privy-wallets.ts states the same reasoning.
 */
 ...
 * submitter.
 *
 * IT READS THE INDEX, and builds one only if none was handed to it. The SDK
 * does expose `getWalletByAddress` — an earlier comment here claimed otherwise
 * and that was wrong — but a per-wallet lookup is N requests where the index is
 * one scan, so the index wins for every N above one. Use the parameter; the
 * fallback exists for callers with a single wallet.
 *
...
export async function createPrivySolanaSigner(
  ...
  const signer: SolanaWalletSubmitter = {
    ...
    async submit(transaction) {
      ...
      // object itself (a base64 string in, {transaction, encoding} out — its
      // services/solana.js says so in as many words). The first version nested
      // a hand-built `params`, which the SDK's own overwrote with
      // {transaction: undefined}, and Privy answered 400 `params.transaction`
      // is required — an error naming a field this file believed it had sent.
      // The `as never` that made that compile is gone with the cause.
      const response = await wallets.solana().signAndSendTransaction(walletId, {
        caip2: (config.caip2 ?? SOLANA_MAINNET_CAIP2) as `${string}:${string}`,
        transaction: serialized.toString("base64"),
        authorization_context: { authorization_private_keys: [config.authorizationKey] },
      });
