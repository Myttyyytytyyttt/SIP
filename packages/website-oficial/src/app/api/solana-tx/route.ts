 ...
 * - after all of those, or right after the compute budget and nowhere else, at
 *   most the Lighthouse checks Phantom adds when it signs on mainnet: account
 *   assertions only, each about one account the transaction already names (ahead
 *   of SaverFi's instructions, only an account they write other than the fee
 *   payer), adding no signer and no writable key (@sip/solana-core
 *   client/lighthouse.ts, the same rule the page applies);
